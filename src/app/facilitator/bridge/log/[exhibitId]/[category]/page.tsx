...
export default function BridgeLogPage() {
  return (
...
function Inner({ user }: { user: User }) {
  ...
  const back = `/facilitator/bridge/exhibit/${exhibitId}`;
  ...
  async function submit(e: React.FormEvent) {
    ...
    router.push(`/facilitator/bridge/exhibit/${exhibitId}?logged=1`);
  }
  ...
  return (
    <Shell user={user} back={back}>
      <form onSubmit={submit} className="space-y-5 pb-28">
        <div className="brutal-card overflow-hidden">
          ...
          <div className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="display text-lg">{meta.label}</div>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {(["low", "medium", "high"] as const).map((u) => (
                <button
                  ...
                  className={`tap py-3 rounded-card border-2 text-sm font-semibold capitalize ${
                    urgency === u
                      ? u === "high"
                        ? "bg-parsec-crimson text-white border-parsec-crimson"
                        : u === "medium"
                        ? "bg-parsec-orange text-white border-parsec-orange"
                        : "bg-parsec-cyan text-parsec-ink border-parsec-cyan"
                      : "bg-white text-parsec-ink border-parsec-ink/10"
                  }`}
                >
                  {u}
                </button>
        ...
        <Section label={`Notes${category === "research" ? " (optional)" : ""}`}>
          <textarea
            value={freeText}
